...
async function getUser(uidStr) {
  ...
  try {
    ...
    return {
      ...
      name: user[1],
      active: user[2],
      lastCheckpoint: user[3],
      lastScanTime: user[4],
      journeyExpiry: user[5],   // 🔥 added journeyExpiry
    };
  } catch (err) {
    return null;
  }
...
// ===== Register user =====
async function registerUser(uidStr, aadhaarHash, name, journeyExpiry) {
  const uid = stringToBytes32(uidStr);
  const tx = contract.methods.registerUser(uid, aadhaarHash, name, journeyExpiry);
  return await signAndSend(tx);
}

// ===== Revoke user =====
async function revokeUser(uidStr) {
  const uid = stringToBytes32(uidStr);
  const tx = contract.methods.revokeUser(uid);
  return await signAndSend(tx);
}

// ===== Log checkpoint scan =====
async function logScan(uidStr, checkpoint) {
  const uid = stringToBytes32(uidStr);
  const tx = contract.methods.logScan(uid, checkpoint);
  return await signAndSend(tx);
}

module.exports = { getUser, registerUser, revokeUser, logScan };
